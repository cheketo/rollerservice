...
class CoreProfile 
{
	...
	public function __construct($ID=0)
	{
		...
		if($ID>0)
		{
			$Data 		= Core::Select("admin_profile","*","profile_id=".$this->ID);
			$this->Data = $Data[0];
		}
	}
	...
	public function GetMenuRelations()
	{
		if(!$this->MenuRelations)
			$this->MenuRelations = Core::Select('relation_menu_profile','*',"profile_id = ".$this->ID);
	...
	{
		if(!$this->Users)
			$this->Users = Core::Select('core_user','*',"profile_id = ".$this->ID." AND status <> 'I'");
		return $this->Users;
	}
	...
	public function GetGroups()
	{
		if(!$this->Groups)
		{
			$Rs 	= Core::Select('admin_group','*',"status = 'A' AND group_id IN (SELECT group_id FROM relation_group_profile WHERE profile_id=".$this->ID.")","title");
			$this->Groups = $Rs;
		}
		return $this->Groups;
	}
...
public function MakeRegs($Mode="List")
	{
		...
		foreach($Rows as $Row)
		{
			$Row		= new CoreProfile($Row['profile_id']);
			$ID 		= $Row->ID;
	...
	protected function InsertSearchField()
	{
		return '<!-- Title -->
          ...
          <div class="input-group">
            ...
            '.Core::InsertElement('select','group','','form-control','',Core::Select('admin_group','group_id,title',"organization_id=".$_SESSION['organization_id']." AND status='A'","title"),'', 'Grupo').'
          </div>';
	}
	...
	public function ConfigureSearchRequest()
	{
		...
		$this->SetWhere("p.organization_id = ".$_SESSION['organization_id']);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public function Insert()
	{
		...
		$NewID		= Core::Insert('admin_profile','organization_id,title,creation_date',$_SESSION['organization_id'].",'".$Title."',NOW()");
		$New 		= new CoreProfile($NewID);
		...
		Core::Update('admin_profile',"image='".$Image."'","profile_id=".$NewID);
		...
		if(!empty($Groups)) Core::Insert('relation_group_profile','profile_id,group_id',$Values);
		$Values = "";
		...
		if(!empty($Menues)) Core::Insert('relation_menu_profile','profile_id,menu_id',$Values);
	}
	...
	public function Update()
	{
		...
		$Insert		= Core::Update('admin_profile',"title='".$Title."',image='".$Image."'","profile_id=".$ID);
		Core::Delete('relation_group_profile',"profile_id = ".$ID);
		Core::Delete('relation_menu_profile',"profile_id = ".$ID);
		foreach($Groups as $Group)
		{
			if(intval($Group)>0)
	...
	{
		$ID	= $_POST['id'];
		Core::Update('admin_profile',"status = 'A'","profile_id=".$ID);
	}
	
	public function Delete()
	{
		$ID	= $_POST['id'];
		Core::Update('admin_profile',"status = 'I'","profile_id=".$ID);
	}
	...
	public function Validate()
	{
	    ...
	    if($ActualTitle)
	    	$TotalRegs  = Core::NumRows('admin_profile','*',"title = '".$Title."' AND title <> '".$ActualTitle."' AND organization_id = ".$_SESSION['organization_id']);
    	else
		    $TotalRegs  = Core::NumRows('admin_profile','*',"title = '".$Title."' AND organization_id = ".$_SESSION['organization_id']);
