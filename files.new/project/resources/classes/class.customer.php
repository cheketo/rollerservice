...
public function MakeRegs($Mode="List")
	...
	protected function InsertSearchField()
	{
		if(!$_GET['country'])
		{
          ...
          <div class="input-group">
            ...
            '.Core::InsertElement('text','country','','form-control','placeholder="Pa&iacute;s"').'
          </div>';
		}
		...
		return '
          ...
          <div class="input-group">
            ...
            '.Core::InsertElement('text','cuit_number','','form-control','placeholder="CUIT"').'
          </div>
          <!-- IIBB -->
          <div class="input-group">
            <span class="input-group-addon order-arrows" order="iibb" mode="asc"><i class="fa fa-sort-alpha-asc"></i></span>
            '.Core::InsertElement('text','iibb','','form-control','placeholder="Ingresos Brutos"').'
          </div>
          <!-- Address -->
          <div class="input-group">
	...
	public function ConfigureSearchRequest()
	{
		$this->SetTable(
			...
			LEFT JOIN core_user f ON (f.user_id = e.broker_id)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public function Insert()
	{
		...
		$NewID			= Core::Insert($this->Table,'type_id,name,cuit,iva,iibb,international,logo,creation_date,created_by,organization_id',"'".$Type."','".$Name."',".$CUIT.",".$IVA.",".$GrossIncome.",'".$International."','".$Image."',NOW(),".$_SESSION['user_id'].",".$_SESSION['organization_id']);
		//echo $this->LastQuery();
		...
		Core::Update($this->Table,"logo='".$Image."'",$this->TableID."=".$NewID);
	...
	public function Update()
	{
		...
		$Update		= Core::Update($this->Table,"name='".$Name."',type_id='".$Type."',cuit=".$CUIT.",iva='".$IVA."',iibb='".$GrossIncome."', logo='".$Image."',updated_by=".$_SESSION['user_id'],$this->TableID."=".$ID);
		//echo $this->LastQuery();
	...
	public function Validate()
	{
	    ...
	    if($ActualName)
	    	$TotalRegs  = Core::NumRows($this->Table,'*',"name = '".$Name."' AND name <> '".$ActualName."'");
    	else
		    $TotalRegs  = Core::NumRows($this->Table,'*',"name = '".$Name."'");
	...
	public function InsertBranchInfo($DeleteInfo=0)
	{
		...
		if($DeleteInfo)
		{
			Core::Delete('customer_agent',"customer_id=".$this->ID);
			Core::Delete('relation_customer_broker',"customer_id=".$this->ID);
			Core::Delete('customer_branch',"customer_id=".$this->ID);
		}
		...
		for($I=1;$I<=$_POST['total_branches'];$I++)
		{
			if($_POST['branch_name_'.$I])
			{
				...
				$BranchID 		= Core::Insert("customer_branch","customer_id,country_id,province_id,region_id,zone_id,name,address,phone,email,website,fax,postal_code,main_branch,lat,lng,creation_date,created_by,organization_id",$this->ID.",".$Branches[$I]['country_id'].",".$Branches[$I]['province_id'].",".$Branches[$I]['region_id'].",".$Branches[$I]['zone_id'].",'".$Branches[$I]['name']."','".$Branches[$I]['address']."','".$Branches[$I]['phone']."','".$Branches[$I]['email']."','".$Branches[$I]['website']."','".$Branches[$I]['fax']."','".$Branches[$I]['postal_code']."','".$Branches[$I]['main_branch']."',".$Branches[$I]['lat'].",".$Branches[$I]['lng'].",NOW(),".$_SESSION['user_id'].",".$_SESSION['organization_id']);
				//echo $this->LastQuery();
				...
				foreach($Agents as $Agent)
				{
					if($Fields)
						$Fields .= "),(";
					$Fields .= $this->ID.",".$BranchID.",'".$Agent['name']."','".$Agent['charge']."','".$Agent['email']."','".$Agent['phone']."','".$Agent['extra']."',NOW(),".$_SESSION['user_id'].",".$_SESSION['organization_id'];
				}
				Core::Insert('customer_agent','customer_id,branch_id,name,charge,email,phone,extra,creation_date,created_by,organization_id',$Fields);
				...
				foreach($Brokers as $Broker)
				{
					if($Broker>0)
					{
						if($Fields)
							$Fields .= "),(";
						$Fields .= $this->ID.",".$BranchID.",".$Broker.",".$_SESSION['organization_id'];
					}
				}
				if($Fields)
					Core::Insert('relation_customer_broker','customer_id,branch_id,broker_id,organization_id',$Fields);
			}
		}
	}
	...
	public function Getbranchmodal($ID=1,$Data=array())
    {
    	...
    	if(empty($Data))
		{
			...
			$Results	= Core::Select('relation_customer_broker','broker_id','branch_id='.$Data['branch_id']);
			foreach($Results as $Broker)
		    {
		      $Brokers .= $Brokers? ','.$Broker['broker_id'] : $Broker['broker_id']; 
		    }
		    $Agents	= Core::Select('customer_agent','*','branch_id='.$Data['branch_id']);
		    $A=0;
		    $AgentsHTML = "";
		    foreach($Agents as $Agent)
		    {
