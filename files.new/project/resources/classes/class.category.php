...
class Category 
{
	...
	public function GetProducts()
	{
		if(!$this->Products)
		{
			$this->Prodcuts = Core::Select("relation_product_category",'product_id',$this->TableID." =".$this->ID);
		}
	...
	{
		$Parents = array();
		$Categories = Core::Select($this->Table,$this->TableID.',title',"status='A' AND organization_id = ".$_SESSION['organization_id'],$Order);
		foreach($Categories as $Category)
		{
			$Children = Core::Select($this->Table,$this->TableID,"status='A' AND parent_id =".$Category['category_id']);
			if(count($Children)>0)
	...
	{
		return Core::Select($this->Table,'*',"status='A' AND organization_id = ".$_SESSION['organization_id'],$Order);
		
	}
	
	public function CalculateCategoryLevel($CategoryID)
	{
		$Parent = Core::Select($this->Table,"parent_id","category_id=".$CategoryID);
	...
	protected function InsertSearchField()
	{
		...
		return '<!-- Name -->
          <div class="input-group">
            ...
            '.Core::InsertElement('text','title','','form-control','placeholder="L&iacute;nea"').'
          </div>
          <!-- Short Name -->
          <div class="input-group">
            <span class="input-group-addon order-arrows sort-activated" order="short_title" mode="asc"><i class="fa fa-sort-alpha-asc"></i></span>
            '.Core::InsertElement('text','short_title','','form-control','placeholder="Nombre Corto"').'
          </div>
          <!-- Parents -->
          <div class="input-group">
            <span class="input-group-addon order-arrows" order="parent" mode="asc"><i class="fa fa-sort-alpha-asc"></i></span>
            '.Core::InsertElement('select','parent','','form-control','',$Parents,'','Cualquier Ubicaci&oacute;n').'
          </div>
          ';
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public function Insert()
	{
		...
		$Parent		= $_POST['parent'];
		if(!$Parent) $Parent = 0;
		$Insert			= Core::Insert($this->Table,'title,short_title,parent_id,creation_date,organization_id,created_by',"'".$Title."','".$ShortTitle."',".$Parent.",NOW(),".$_SESSION['organization_id'].",".$_SESSION['user_id']);
		//echo $this->LastQuery();
	}	
	...
	public function Update()
	{
		...
		$Update		= Core::Update($this->Table,"title='".$Title."',short_title='".$ShortTitle."',parent_id=".$Parent.",updated_by=".$_SESSION['user_id'],$this->TableID."=".$ID);
		//echo $this->LastQuery();
	}
	...
	public function Validate()
	{
	    ...
	    if($ActualName)
	    	$TotalRegs  = Core::NumRows($this->Table,'*',"parent_id = ".$ParentID." AND title = '".$Name."' AND title<> '".$ActualName."'");
    	else
		    $TotalRegs  = Core::NumRows($this->Table,'*',"parent_id = ".$ParentID." AND title = '".$Name."'");
