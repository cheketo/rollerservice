...
class GeolocationProvince 
{
	...
	var $Zones 		= array();
	var $ID;
	var $Table		= 'admin_country_province';
	var $TableID	= 'province_id';
	...
	public function GetData()
	{
		if(!$this->Data)
		{
			$Data 		= Core::Select($this->Table,"*",$this->TableID."=".$this->ID);
			$this->Data = $Data[0];
			if($this->Data['country_id'])
			{
				$Data	= Core::Select('admin_country','*','country_id = '.$this->Data['country_id']);
				$this->Data['country'] = $Data[0]['title'];
			}
		}
		return $this->Data;
	}
	
	public function GetZones($Province)
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{	
		...
		$Insert		= Core::Insert('admin_country','title,creation_date',"'".$Title."',NOW()");
	}
	
	public function Update()
	{
		$ID 	= $_POST['id'];
		$Edit	= new GeolocationProvince($ID);
		$Title		= ucfirst($_POST['title']);
		$Update		= Core::Update('admin_country',"title='".$Title."'","country_id=".$ID);
		//echo $this->LastQuery();
	...
	{
		$ID	= $_POST['id'];
		Core::Update('admin_country',"status = 'A'","country_id=".$ID);
	}
	
	public function Delete()
	{
		$ID	= $_POST['id'];
		Core::Update('admin_country',"status = 'I'","country_id=".$ID);
	}
	...
	public function Validate()
	{
	    ...
	    if($ActualTitle)
	    	$TotalRegs  = Core::NumRows('admin_country','*',"title = '".$Title."' AND title <> '".$ActualTitle."'");
    	else
		    $TotalRegs  = Core::NumRows('admin_country','*',"title = '".$Title."'");
	...
	public function Fillzones()
	{
		...
		//$Zones = Core::Select("admin_country_zone","*","status = 'A' AND province_id = ".$Province);
		$HTML = '<span class="input-group-addon"><i class="fa fa-map-o"></i></span>'.Core::InsertElement('select','zone','','form-control chosenSelect','data-placeholder="Seleccione una Zona"',Core::Select('admin_country_zone','zone_id,title',"status<>'I' AND country_id = 54 AND province_id=".$Province),' ','');
		echo $HTML;
	}
}
